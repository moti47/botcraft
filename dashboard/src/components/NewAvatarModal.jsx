...
import { useCreateAvatar, useAvatarPreview, useImproveField, proxyImage } from '../BotCraftData'
...
export const NewAvatarModal = ({ isOpen, onClose, onSuccess, uiLanguage = 'EN' }) => {
  ...
  const improveMutation = useImproveField()
  // Track which field is currently being improved so we can show a spinner
  // on just that one button instead of disabling everything.
  ...
  const improveField = async (fieldName) => {
    ...
    try {
      ...
      if (result?.improved) {
        setForm((f) => ({ ...f, [fieldName]: result.improved }))
        if (['name', 'physical_description'].includes(fieldName)) setImageStale(true)
      }
    } catch (err) {
      console.error(`Improve ${fieldName} failed:`, err)
    } finally {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Box title="👤 Personal Details" subtitle="Generated by AI, fully editable">
              <Field
                ...
                improveProps={{
                  show: !!form.name.trim(),
                  loading: improvingField === 'name',
                  disabled: isWorking,
                  onClick: () => improveField('name'),
                }}
              >
              ...
              <Field
                label="Bio"
                hint="one-line tagline"
                improveProps={{
                  show: !!form.bio.trim(),
                  loading: improvingField === 'bio',
                  disabled: isWorking,
                  onClick: () => improveField('bio'),
                }}
              >
                <textarea
              ...
              <Field
                label="Life story"
                hint="multi-paragraph backstory in chosen language"
                improveProps={{
                  show: !!form.life_story.trim(),
                  loading: improvingField === 'life_story',
                  disabled: isWorking,
                  onClick: () => improveField('life_story'),
                }}
              >
                <textarea
              ...
              <Field
                label="Physical description (English)"
                hint="drives the portrait — edit to change the face"
                improveProps={{
                  show: !!form.physical_description.trim(),
                  loading: improvingField === 'physical_description',
                  disabled: isWorking,
                  onClick: () => improveField('physical_description'),
                }}
              >
                <textarea
              ...
              <Field
                ...
                hint="anything else for the LLM to know"
                improveProps={{
                  show: !!form.custom_instructions.trim(),
                  loading: improvingField === 'custom_instructions',
                  disabled: isWorking,
                  onClick: () => improveField('custom_instructions'),
                }}
              >
                <textarea
...
const Field = ({ label, hint, required, children, improveProps }) => (
  <div>
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4, gap: 8 }}>
      ...
      {improveProps?.show && (
        <button
          type="button"
          onClick={improveProps.onClick}
          disabled={improveProps.loading || improveProps.disabled}
          title="Send the current text to the AI and get a polished version back"
          style={{
            padding: '3px 9px',
            fontSize: 10,
            fontWeight: 700,
            background: improveProps.loading ? 'var(--surface-2)' : 'var(--brand-gradient)',
            color: improveProps.loading ? 'var(--text-muted)' : '#fff',
            border: 'none',
            borderRadius: 999,
            cursor: improveProps.loading || improveProps.disabled ? 'wait' : 'pointer',
            opacity: improveProps.disabled ? 0.5 : 1,
            boxShadow: improveProps.loading ? 'none' : '0 2px 6px rgba(124,58,237,0.35)',
            whiteSpace: 'nowrap',
          }}
        >
          {improveProps.loading ? '✨ …' : '✨ Improve'}
        </button>
      )}
    </div>
    {children}
